...
<form action="" method="POST">
				...
				<div class="card-header">
					<center><h3 class="text text-primary">Add Marks Second Term Level3 SOD</h3></center>
				</div>
				<div class="card-body">
					Department
					...
					<option>Select Student</option>						
						<?php 
                        $conn=mysqli_connect("localhost","root","","sms");
                        $select="SELECT st_id,student_code,firstname,lastname FROM student WHERE department='SOD' AND level3='l3'";
                        $query=mysqli_query($conn,$select);
                        while ($data=mysqli_fetch_array($query)) {
                        ?>
					...
					<option>Select Module</option>						
						<?php 
                        $conn=mysqli_connect("localhost","root","","sms");
                        $select="SELECT module_id,module_code,module_name FROM modules WHERE department_name='SOD' AND level='l3'";
                        $query=mysqli_query($conn,$select);
                        while ($data=mysqli_fetch_array($query)) {
                        ?>
