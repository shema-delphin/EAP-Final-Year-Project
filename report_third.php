...
<center><p><b>Third Term</b></p></center>
...
<table>
	...
	$st_id=$_GET['st_id'];
	$select=mysqli_query($conn,"SELECT * FROM report_third WHERE st_id='$st_id' GROUP BY st_id");
	...
	if ($num>0) {
		...
		<tr bgcolor="yellow">
			...
			<td><b><?php echo $data['behavior_third_marks'] ?></b></td>
...
<table border="">
	...
	$conn=mysqli_connect("localhost","root","","sms");
	$st_id=$_GET['st_id'];
	$select=mysqli_query($conn,"SELECT * FROM report_third WHERE st_id='$st_id'");
	$num=mysqli_num_rows($select);
