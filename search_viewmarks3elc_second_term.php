...
<form action="search_viewmarks5sod_second_term.php" method="GET">
	...
	$select=mysqli_query($conn,"SELECT * FROM sub22 WHERE department_name='ELC' AND level='L3' AND student_code LIKE '%$student_code%'");
	...
	if ($num>0) {
		...
		<tr>
			...
			<td><a onclick="return confirm('Are you Sure you want to Delete?'); " class="btn btn-danger" href="deletemarksl3elc_second_term.php?marks_id=<?php echo $data['marks_id'] ?>">Delete</a></td>

			<td><a class="btn btn-success" href="updatemarksl3elc_second_term.php? marks_id=<?php echo $data['marks_id'] ?>">Update</a></td>
